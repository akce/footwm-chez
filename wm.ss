(library (wm)
  ...
  (define on-configure-request
    (lambda (ev)
      (let ([g (icccm.on-configure-request ev)])
        (display (format "#x~x ConfigureRequest ~a~n" (xconfigurerequestevent-wid ev) g))
        (when (ewmh.dock-window? (xconfigurerequestevent-wid ev))
            ...
            (if (memq wid (ewmh.client-list))
              (let ([state (icccm.get-wm-state wid)])
                ...
                (when (or (not state) (not (eq? state icccm.IconicState)))
                  ...
                  (op.arrange-windows)))
              ;; Window could be a dock app.
              (begin
                ;; Update workarea: assuming window was a dock as it wasn't in the client list.
